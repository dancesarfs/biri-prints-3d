...
6. **Botão "Pedir personalização"** no card do pedido (só aparece com algo pendente) — gera uma mensagem de WhatsApp dedicada (`textoPedirPersonalizacao`/`enviarPedirPersonalizacao`, mesmo mecanismo do "Enviar por WhatsApp"/"Avisar" — `enviarWhatsAppParaCliente`) listando só as peças que ainda faltam. Pode ser clicado de novo quantas vezes precisar (é só um follow-up, sem estado de "já enviei").
...
- `test_personalizacao_orcamento_standalone.js`: escolha de cor/texto por unidade no orçamento (matriz), atalho "aplicar a todas", conversão em pedido liberada mesmo com personalização pendente, resumo em Orçamentos/Pedidos, bloco especial na mensagem de "pedido recebido", reset ao copiar orçamento, badge/filtro/"Pedir personalização"/formulário inline e bloqueio da transição "Aberto → Em produção" em Pedidos (Etapa 3), e a garantia de não bloquear retroativamente um pedido já avançado antes dessa checagem existir.
