...
4. **Validação só na conversão em pedido** (nunca ao salvar o orçamento, que pode ficar incompleto — é só um rascunho de venda): `personalizacaoPendente(o)` bloqueia com um toast claro se faltar cor em alguma unidade, ou texto em alguma unidade cuja parte marcou texto como obrigatório; texto opcional nunca bloqueia.
...
- `test_personalizacao_orcamento_standalone.js`: escolha de cor/texto por unidade no orçamento (matriz), atalho "aplicar a todas", bloqueio da conversão em pedido, resumo em Orçamentos/Pedidos, bloco especial na mensagem de "pedido recebido", reset ao copiar orçamento.
